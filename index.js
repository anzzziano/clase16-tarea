...
app.get("/frutas/importe/:importe", async (req, res) => {
  const importe = parseInt(req.params.importe) || 0;
  const client = await connectToMongoDB();
  if (!client) {
    res.status(500).send("Error client");
    return;
  ...
  const db = client.db("sample_mflix");
  const fruta = await db.collection("frutas").findOne({ importe: importe });
...
app.get("/frutas/nombre/:nombre", async (req, res) => {
  const nombre = req.params.nombre || 0;
  const client = await connectToMongoDB();
  if (!client) {
    res.status(500).send("Error client");
    return;
  ...
  const db = client.db("sample_mflix");
  const fruta = await db.collection("frutas").findOne({ nombre: nombre });
